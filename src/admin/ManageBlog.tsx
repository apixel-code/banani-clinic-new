...
export default function ManageBlog() {
  ...
  const load = () => {
    setLoading(true);
    supabase.from('blog_posts').select('id,title,slug,category,published,published_at,author').order('created_at', { ascending: false }).then(({ data }) => { setPosts(data || []); setLoading(false); });
  };
